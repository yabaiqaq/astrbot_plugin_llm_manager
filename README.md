...
```
/llm import          # 首次安装插件时使用，从系统配置导入所有供应商
/llm resync          # 后台手动删改模型供应商配置后，用此命令刷新模型列表

/llm list            # 查看所有模型 （渲染成卡片图片展示，需安装pillow）
/llm use 3           # 切换到序号 3 的模型 （全局生效）

/llm test            # 测试当前模型延迟
/llm test 4          # 测试4号模型延迟
/llm test 3 5 8      # 批量测试3，5，8号模型延迟(最多支持10个)

/llm stats          # Token消耗统计 默认最近 7 天
/llm stats 1d       # 最近 1 天
/llm stats 7d       # 最近 7 天
/llm stats 30d      # 最近 30 天
/llm stats all      # 全部历史（受 30 天保留期限制）
```
